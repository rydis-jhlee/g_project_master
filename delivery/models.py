...
class Delivery(models.Model):
    ...
    username = models.CharField(max_length=150, null=True)                 # 배송기사ID(직접수령이면 구매자ID, 배달이면 배송기사ID)
    mobile = models.CharField(max_length=150)             # 배송기사 연락처(직접수령이면 구매자 전화번호, 배달이면 배송기사 전화번호)
    ...
    group_id = models.ForeignKey('user.DeliveryGroup', null=True, on_delete=models.SET_NULL, db_column='group_id')
